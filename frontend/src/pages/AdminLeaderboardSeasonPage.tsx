...
export function AdminLeaderboardSeasonPage() {
  ...
  return <section className="admin-page leaderboard-season-admin-page season-workspace">
    <div className="page-header ui-v2-page-header"><div><h1>榜单管理</h1></div>{isRoot && (!currentSeason || scheduled) && <button className="button primary" type="button" onClick={openEditor}>{currentSeason ? "编辑赛季" : "创建赛季"}</button>}</div>
...
function AuditSection({ title, children }: { title: string; children: ReactNode }) { return <section className="workspace-section audit-section"><div className="workspace-section-header static"><div><h2>{title}</h2></div></div><div className="workspace-section-body">{children}</div></section>; }
...
function statusLabel(status: number) { return ["", "未开始", "进行中", "已冻结", "公示中", "已归档"][status] ?? "未知"; }
