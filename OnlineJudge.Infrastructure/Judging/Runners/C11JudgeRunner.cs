...
public class C11JudgeRunner(IJudgeSandbox judgeSandbox, C11FunctionJudgeCodeBuilder functionJudgeCodeBuilder) : IJudgeRunner
{
    ...
    {
        return language == JudgeLanguage.C11;
    }
    ...
    private async Task<JudgeResult> RunFunctionAsync(JudgeRequest request, CancellationToken cancellationToken)
    {
        if (ContainsMainFunction(request.SourceCode))
        {
            return new JudgeResult
            {
                ...
                ErrorMessage = "函数式题目不需要编写 main，请只提交目标函数实现。"
            };
        }
