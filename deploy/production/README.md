...
- PostgreSQL and Redis data: operator-created Docker volumes named by `POSTGRES_VOLUME_NAME` and `REDIS_VOLUME_NAME`. They are external to Compose and must already exist before infrastructure starts.
...
Before adopting an existing PostgreSQL or Redis container into this Compose project, inspect its mounts with `docker inspect` and put the exact existing volume names into `POSTGRES_VOLUME_NAME` and `REDIS_VOLUME_NAME`. Verify the backup and both volumes before stopping the old containers. Bring the old Compose project down without `--volumes`, then start this project; the external-volume contract preserves the existing data and makes a misspelled or missing volume a hard failure instead of silently creating an empty database. Do not run two PostgreSQL or Redis containers against the same volume.
